...
if(!function_exists('readCSV')){
     ...
     * @return array
     */
    function readCSV($filePath) :array
    {
        try {
            ...
            foreach ($reader->getSheetIterator() as $sheet) {
                // Itera sobre cada linha na planilha
...
if(!function_exists('readXLSX')){
    /**
     * Função responsável por ler um arquivo XLSX e retornar seu conteúdo
     *
     * @param string $filePath Caminho do arquivo
     *
     * @access public
     * @return array
     */
    function readXLSX($filePatch) :array
    {
        try {
            $reader = ReaderEntityFactory::createXLSXReader();
            $reader->setShouldFormatDates(true);
            $reader->open($filePatch);
...
if(!function_exists('createDre')){
    /**
     * Função responsável transformar os dados de DRE para tabela
     *
     * @param object $data Dados do relatório DRE {
     *          description: Descrição da categoria atual
     *          total: Valor total da categoria
     *          months: (Array) Valores por mês (Ex.: [ '01' => 0.00, '02' => 150.00, ..., '12' => '300.00' ])
     *          Children: (Object) Caso a categoria possua filho, deve conter os dados no mesmo padrão da categoria pai (Opcional)
     * }
     *
     * @access public
     * @return string
     */
    function createDre($data, $type, $spaces = 0){
        $space = '';
        if ($spaces > 0) {
            for ($i = 0; $i < $spaces; $i++) {
                $space .= '&nbsp;&nbsp;&nbsp;&nbsp;';
        ...
        if ($type == 1) {
            ...
            <tr style="border: 1px solid #000;">
                <td style="border: 1px solid #000;">'.$space.$data->description.'</td>
                <td style="border: 1px solid #000;">'.formatFloatToValue($data->total).'</td>
            </tr>';
        } else {
            $tr = '
            <tr style="border: 1px solid #000;">
                <td style="border: 1px solid #000;">'.$space.$data->description.'</td>
                <td style="border: 1px solid #000;">'.formatFloatToValue($data->months['01']).'</td>
                <td style="border: 1px solid #000;">'.formatFloatToValue($data->months['02']).'</td>
                <td style="border: 1px solid #000;">'.formatFloatToValue($data->months['03']).'</td>
                <td style="border: 1px solid #000;">'.formatFloatToValue($data->months['04']).'</td>
                <td style="border: 1px solid #000;">'.formatFloatToValue($data->months['05']).'</td>
                <td style="border: 1px solid #000;">'.formatFloatToValue($data->months['06']).'</td>
                <td style="border: 1px solid #000;">'.formatFloatToValue($data->months['07']).'</td>
                <td style="border: 1px solid #000;">'.formatFloatToValue($data->months['08']).'</td>
                <td style="border: 1px solid #000;">'.formatFloatToValue($data->months['09']).'</td>
                <td style="border: 1px solid #000;">'.formatFloatToValue($data->months['10']).'</td>
                <td style="border: 1px solid #000;">'.formatFloatToValue($data->months['11']).'</td>
                <td style="border: 1px solid #000;">'.formatFloatToValue($data->months['12']).'</td>
            </tr>';
        }
        if (isset($data->Children)) {
            foreach ($data->Children as $son) {
                $tr .= createDre($son, $type, $spaces);
            }
        }
...
if (!function_exists('calcUteis')) {
     ...
     * @param array $feriados Array com os feriados do ano/mês, as datas devem vir no formato Y-m-d (Opcional)
     *
     * @access public
     * @return int
     */
    function calcUteis(string $date_start, string $date_end, $feriados = [])
    {
        ...
        $dtFim = new DateTime($date_end);
        if (in_array(date('H:i:s', strtotime($date_start)), ['08:00:00', '13:30:00'])) {
            $dtFim = new DateTime(date('Y-m-d H:i:s', strtotime('+1 minute', strtotime($date_end))));
        }

        $intervaloEmMinuto = new DateInterval('PT1M');
        $periodo = new DatePeriod($dtIni, $intervaloEmMinuto, $dtFim);
        foreach ($periodo as $data) {
            /* @var $data \DateTime */
            $dataEmMinuto = clone $data;

            /**Caso seja fim de semana ou feriado, ignora */
            if (in_array(date('w', $data->getTimestamp()), [0, 6]) || in_array(date('Y-m-d', $data->getTimestamp()), $feriados)) {
                continue;
            }
            ...
            $fimDoSegundoTurno = clone $dataEmMinuto->setTime(17, 30, 0);
...
if (!function_exists('calculateDuration')) {
     ...
     * @param array $feriados Array com os feriados do ano/mês, as datas devem vir no formato Y-m-d (Opcional)
     */
    function calculateDuration($date, $duration, $feriados = []) {
        ...
        for ($i = 1; $i <= $dias; $i++) {
            ...
            if (in_array(date('w', strtotime($data_final)), [0, 6]) || in_array(date('Y-m-d', strtotime($data_final)), $feriados)) {
                $dias++;
            }
        }
...
if (!function_exists('incrementBussinessDay')) {
     ...
     * @param array $feriados Array com os feriados do ano/mês, as datas devem vir no formato Y-m-d (Opcional)
     */
    function incrementBussinessDay($date, $dias, $feriados = []) {;
        $data_final = $date;
        for ($i = 1; $i <= $dias; $i++) {
...
if (!function_exists('decrementBussinessDay')) {
     ...
     * @param array $feriados Array com os feriados do ano/mês, as datas devem vir no formato Y-m-d (Opcional)
     */
    function decrementBussinessDay($date, $dias, $feriados = []) {;
        $data_final = $date;
        for ($i = 1; $i <= $dias; $i++) {
            $data_final = date('Y-m-d', strtotime($data_final.' -1 day'));
